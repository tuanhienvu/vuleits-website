...
/** Public JSON shape from GET `company/contact` (see `apiPath`); `type` matches company profile social platforms. */
export type PublicSocialLink = { type: string; url: string };

const PLATFORM_NAMES: Record<SocialPlatformId, string> = {
  facebook: 'Facebook',
  youtube: 'YouTube',
  instagram: 'Instagram',
  tiktok: 'TikTok',
...
function isPlatformId(v: string): v is SocialPlatformId {
    ...
    case 'facebook':
      return (
        <svg className={cls} viewBox="0 0 24 24" aria-hidden>
          <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.413c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
        </svg>
      );
    case 'youtube':
      return (
...
type CompanySocialLinksProps = {
  links: PublicSocialLink[];
  /** Show a small heading above the icons (e.g. on Contact page). */
  showHeading?: boolean;
  className?: string;
...
export default function CompanySocialLinks({
  ...
  return (
    <div className={className}>
      {showHeading ? (
        <h4 className="text-white font-semibold mb-3">{t('contact.followUs')}</h4>
      ) : null}
      <ul
