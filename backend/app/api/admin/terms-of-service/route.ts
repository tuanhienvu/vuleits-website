...
export async function GET(req: Request) {
  ...
  const payload = parseLegalPageJson(row?.value ?? null, 'terms');
...
export async function PUT(req: Request) {
  ...
  await prisma.siteSetting.upsert({
    where: { key: TERMS_OF_SERVICE_SETTING_KEY },
    create: { key: TERMS_OF_SERVICE_SETTING_KEY, value: serializeLegalPagePayload(payload) },
