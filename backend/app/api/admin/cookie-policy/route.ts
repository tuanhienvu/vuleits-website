...
export async function GET(req: Request) {
  ...
  const payload = parseLegalPageJson(row?.value ?? null, 'cookie');
...
export async function PUT(req: Request) {
  ...
  await prisma.siteSetting.upsert({
    where: { key: COOKIE_POLICY_SETTING_KEY },
    create: { key: COOKIE_POLICY_SETTING_KEY, value: serializeLegalPagePayload(payload) },
