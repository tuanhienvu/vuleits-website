...
export async function GET(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  // Respect publish window
  if (article.publishedAt && effectiveDate > now) {
    return NextResponse.json({ error: 'Not found' }, { status: 404 });
  }
  if (!article.publishedAt && article.startDate && effectiveDate > now) {
    return NextResponse.json({ error: 'Not found' }, { status: 404 });
  }

  const contentHtml = sanitizeNewsContentHtml(article.content);

  // Related by category first
